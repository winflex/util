...
public class ClasspathUrlFinder {
	...
	/**
	 * Find the classpath URLs for a specific classpath resource. The classpath URL
	 * is extracted from loader.getResources() using the baseResource.
	 *
	 * @param baseResource
	 * @return
	 */
	...
	public static URL[] findClassPaths() {
		...
		while (tokenizer.hasMoreTokens()) {
			...
			try {
				...
				if (!fp.exists())
					continue;// 跳过不存在的路径，以支持a/b/c/*这种写法
	...
	private static void searchDir(List<URL> list, File fp) throws MalformedURLException {
		if (fp == null)
			return;
