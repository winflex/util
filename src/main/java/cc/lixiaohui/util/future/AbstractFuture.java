...
public class AbstractFuture<V> implements IFuture<V> {
    ...
    private final Collection<IFutureListener<V>> listeners = new CopyOnWriteArrayList<IFutureListener<V>>();
    ...
    @Override
    public IFuture<V> addListener(IFutureListener<V> l, Executor executor) {
        listen2Executor.put(l, executor);
        addListener(l);
        return this;
    }
    ...
    public IFuture<V> removeListener(IFutureListener<V> listener) {
        ...
        if (!isDone()) {
            ...
            listen2Executor.remove(listener);
        }
    ...
    private IFuture<V> await0(boolean interruptable) throws InterruptedException {
        if (!isDone()) { // 若已完成就直接返回了
            ...
            if (interrupted) {
                // 为什么这里要重新设置中断标志位？因为从wait方法返回后, 中断标志是被clear了的,
                // 这里重新设置以便让客户端知道这里被中断了。
    ...
    private void notifyListener(final IFutureListener<V> l) {
        try {
            Executor executor = listen2Executor.get(l);
            if (executor == null) {
                // 没有指定的线程池, 则当前线程执行回调方法s
